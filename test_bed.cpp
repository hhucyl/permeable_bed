#include "./lbm/Domain.h"


struct myUserData
{
    std::ofstream oss_ss;
    double ua;
};


void Initial(LBM::Domain &dom, double rho, Vec3_t &v0, Vec3_t&g0)
{
    for(size_t ix=0; ix<dom.Ndim(0); ix++)
    for(size_t iy=0; iy<dom.Ndim(1); iy++)
    for(size_t iz=0; iz<dom.Ndim(2); iz++)
    {
        dom.Rho[ix][iy][iz] = rho;
        dom.Vel[ix][iy][iz] = 0.0, 0.0, 0.0;
        dom.BForce[ix][iy][iz] = g0;
        for(size_t k=0; k<dom.Nneigh; ++k)
        {
            dom.F[ix][iy][iz][k] = dom.Feq(k,rho,v0);            
            dom.Ftemp[ix][iy][iz][k] = dom.Feq(k,rho,v0);            
        }
        
    }
    dom.Rho0 = rho;
}

void FreeSlip(LBM::Domain &dom,  void *UD)
{
    size_t nx = dom.Ndim(0);
    size_t ny = dom.Ndim(1);
    size_t nz = dom.Ndim(2);
    
    #ifdef USE_OMP
    #pragma omp parallel for schedule(static) num_threads(dom.Nproc)
    #endif
    for(size_t ix=0; ix<nx; ix++)
    for(size_t iy=0; iy<ny; iy++)
    {
        // std::cout<<"ix "<<ix<<"iy "<<iy<<std::endl;
        double *f = dom.F[ix][iy][nz-1];
        f[6]  = f[5];
        f[10] = f[16];
        f[12] = f[18];
        f[15] = f[9];
        f[17] = f[11];
    }
}

void InOut(LBM::Domain &dom,  void *UD)
{
    myUserData &dat = (*static_cast<myUserData *> (UD));
    size_t nx = dom.Ndim(0);
    size_t ny = dom.Ndim(1);
    size_t nz = dom.Ndim(2);
    size_t Nneigh = dom.Nneigh;
    #ifdef USE_OMP
    #pragma omp parallel for schedule(static) num_threads(dom.Nproc)
    #endif
    for(size_t iz=0; iz<nz-1; iz++)
    {
        double z = (double) iz -0.5;
        double Lz = nz-1-0.5;
        double uz = 1.5*dat.ua/(Lz*Lz)*(2.0*Lz-z)*z;
        for(size_t iy=0; iy<ny; iy++)
        {
            double *f = dom.Ftemp[0][iy][iz];
            double *f1 = dom.Ftemp[1][iy][iz];
            double rho1 = dom.Rho[1][iy][iz];
            Vec3_t vel1 = dom.Vel[1][iy][iz];
            double rho = dom.Rho[0][iy][iz];
            Vec3_t vel = dom.Vel[0][iy][iz];
            
            if(dom.IsSolid[0][iy][iz]) continue;
            double y = (double) iy -0.5;
            double Ly = ny-1-1;
            double uu = 4.0*uz/(Ly*Ly)*(Ly-y)*y;
            Vec3_t vv (uu,0.0,0.0);

            for(size_t k=0; k<Nneigh; k++)
            {
                // if(k==1&&k==5&&k==7&&k==13&&k==15)
                f[k] = dom.Feq(k,rho1,vv) + f1[k] - dom.Feq(k,rho1,vel1);
                // f[k] = dom.Feq(k,rho1,vv);
            }
            //BForce[ix][iy][iz] = OrthoSys::O;
            vel = OrthoSys::O;
            rho = 0.0;
            for (size_t k=0;k<Nneigh;k++)
            {
                rho +=  f[k];
                vel +=  f[k]*dom.C[k];
            }
            vel *= dom.Cs/rho;

        }
    }
    #ifdef USE_OMP
    #pragma omp parallel for schedule(static) num_threads(dom.Nproc)
    #endif
    for(size_t iz=0; iz<nz; iz++)
    {
        for(size_t iy=0; iy<ny; iy++)
        {
            double *f = dom.Ftemp[nx-1][iy][iz];
            double *f1 = dom.Ftemp[nx-2][iy][iz];
            double rho = dom.Rho[nx-1][iy][iz];
            Vec3_t vel = dom.Vel[nx-1][iy][iz];
            if(dom.IsSolid[nx-1][iy][iz]) continue;
           

            for(size_t k=0; k<Nneigh; k++)
            {
                f[k] = f1[k];
            }
            //BForce[ix][iy][iz] = OrthoSys::O;
            vel = OrthoSys::O;
            rho = 0.0;
            for (size_t k=0;k<Nneigh;k++)
            {
                rho +=  f[k];
                vel +=  f[k]*dom.C[k];
            }
            vel *= dom.Cs/rho;
            

        }
    }

    // #ifdef USE_OMP
    // #pragma omp parallel for schedule(static) num_threads(dom.Nproc)
    // #endif
    // for(size_t iz=0; iz<nz; iz++)
    // {
    //     for(size_t iy=0; iy<ny; iy++)
    //     {
            
    //         double *f1 = dom.F[nx-2][iy][iz];
    //         double rho1 = dom.Rho[nx-2][iy][iz];
    //         Vec3_t vel1 = dom.Vel[nx-2][iy][iz];
    //         double *f = dom.F[nx-1][iy][iz];
    //         double rho = dom.Rho[nx-1][iy][iz];
    //         Vec3_t vel = dom.Vel[nx-1][iy][iz];
            
    //         if(dom.IsSolid[nx-1][iy][iz]) continue;
            

    //         for(size_t k=0; k<Nneigh; k++)
    //         {
    //             f[k] = dom.Feq(k,1.0,vel1) + f1[k] - dom.Feq(k,rho1,vel1);
    //         }
    //         //BForce[ix][iy][iz] = OrthoSys::O;
    //         vel = OrthoSys::O;
    //         rho = 0.0;
    //         for (size_t k=0;k<Nneigh;k++)
    //         {
    //             rho +=  f[k];
    //             vel +=  f[k]*dom.C[k];
    //         }
    //         vel *= dom.Cs/rho;

    //     }
    // }
}

void Report(LBM::Domain &dom, void *UD)
{
    myUserData &dat = (*static_cast<myUserData *> (UD));
    size_t nx = dom.Ndim(0);
    size_t ny = dom.Ndim(1);
    size_t nz = dom.Ndim(2);
    double dx = dom.dx;
    double nu = dom.Nu;
    if(dom.Time <1e-6)
    {
        String fs;
        // fs.Printf("%s_%d_%g.out","R",dat.bbtype,dom.Tau);
        fs.Printf("%s.out","R");
        dat.oss_ss.open(fs.CStr(),std::ios::out);
        dat.oss_ss<<Util::_10_6<<"Time"<<Util::_8s<<"u_ref"<<Util::_8s<<"u"<<Util::_8s<<"r\n";
    }else{
        
        
    }
}

using namespace std;

int main (int argc, char **argv) try
{
    size_t collidetype = 0;
    CollideMethod methodc = SRT;
    if(collidetype == 0)
    {
        methodc = SRT;
        // ptr2collide = &LBM::Domain::CollideSRT;
    }else if(collidetype == 1){
        methodc = MRT;
        // ptr2collide = &LBM::Domain::CollideMRT;
        // ptr2meq = &LBM::Domain::MeqD2Q9;         
    }else{
        throw new Fatal("Collide Type is NOT RIGHT!!!!!");    
    }
    size_t bbtype = 0;    
    size_t Nproc = 12;
    size_t h = 5;
    double Re = 39000;
    double ua = 0.24;
    double H = 0.160;
    double ttt = 1e-4;//t/tlbm
    double Tf = 1e5;
    if(argc>=2) bbtype = atoi(argv[1]); 
    if(argc>=3) Re = atof(argv[2]);
    if(argc>=4) Tf = atof(argv[3]);     
    if(argc>=5) Nproc = atoi(argv[4]); 
    double R = (double) h;
    size_t LL = 5*h;
    size_t nx = 10*h+ 4*LL;
    size_t ny = 10*h+2;
    size_t nz = 12*h+1+std::ceil(H/0.038*2*h);//0.12/0.038 = 3.1579
    double lll = 0.038/((double) 2*h);//L/Llbm
    double dx = 1.0;
    double dt = 1.0;
    double nu = H*ua/Re*ttt/(lll*lll);
    // nu =0.01;
    double uua = ua/lll*ttt;
    Vec3_t pos(0,0,0);
    double rho0 = 1.0;
    Vec3_t v0(0.0,0.0,0.0);
    Vec3_t g0(0.0,0.0,0.0);
    std::cout<<"nu "<<nu<<std::endl;
    std::cout<<"U "<<1.5*uua<<std::endl;
    std::cout<<"L/Llbm "<<lll<<std::endl;
    std::cout<<"t/tlbm "<<ttt<<std::endl;
    std::cout<<"Nx "<<nx<<std::endl;
    std::cout<<"Ny "<<ny<<std::endl;
    std::cout<<"Nz "<<nz<<std::endl;
    LBM::Domain dom(D3Q19,SRT,nu,iVec3_t(nx,ny,nz),dx,dt);
    dom.Nproc = Nproc;
    dom.Sc = 0.17;
    if(bbtype == 0)
    {
        dom.MethodB = SBB;
    }else if(bbtype == 1){
        dom.MethodB = LIBB;        
    }else if(bbtype == 2){
        dom.MethodB = QIBB;
    }else if(bbtype == 3){
        dom.MethodB = MR;
    }else if(bbtype == 4){
        dom.MethodB = CLI;            
    }else{
        throw new Fatal("Collide Type is NOT RIGHT!!!!!");    
    }
    dom.SetBounceBack();
    if(dom.Tau<0.53&&dom.MethodC == MRT)
    {
        if(dom.Nneigh == 19)
        {
            dom.S = 0, 1.19, 1.4, 0, 1.2, 0, 1.2, 0, 1.2, 1/dom.Tau, 1.4, 1/dom.Tau, 1.4, 1/dom.Tau, 1/dom.Tau, 1/dom.Tau, 1.98, 1.98, 1.98;
            dom.we = 0.0;
            dom.wej = -475.0/63.0;
            dom.wxx = 0;
            std::cout<<"Addjust the S"<<std::endl;
        }else if(dom.Nneigh == 15)
        {
            dom.S = 0.0,1.6,1.2,0.0,1.6,0.0,1.6,0.0,1.6,1.0/dom.Tau,1.0/dom.Tau,1.0/dom.Tau,1.0/dom.Tau,1.0/dom.Tau,1.2;
        }
    }
    myUserData my_dat;
    dom.UserData = &my_dat;
    my_dat.ua = uua;
    //debug mod
    // dom.Isq = true;
    // dom.IsF = false;
    // dom.IsFt = false;
    //boundary
    for(size_t ix=0; ix<nx; ix++)
    for(size_t iz=0; iz<nz; iz++)
    {
        dom.IsSolid[ix][0][iz] = true;
        dom.IsSolid[ix][ny-1][iz] = true;
        
    }
    for(size_t ix=0; ix<nx; ix++)
    for(size_t iy=0; iy<ny; iy++)
    {
        dom.IsSolid[ix][iy][0] = true;
    }

    for(size_t ipy=0; ipy<5; ipy++)
    for(size_t ipz=0; ipz<6; ipz++)
    for(size_t ipx=0; ipx<5; ipx++)
    {
        pos = (2*ipx+1)*R+LL,(2*ipy+1)*R + 1,(2*ipz+1)*R + 1;
        dom.AddSphereQ(pos,R);
    }
    //initial
    Initial(dom,rho0,v0,g0);


    Tf = 200;
    double dtout = 1;
    char const * TheFileKey = "test_bed";
    //solving
    dom.StartTime = std::clock();
    dom.StartSolve();
    double tout = 0;
    while(dom.Time<Tf)
    {
        
        if (dom.Time>=tout)
        {
            
            String fn;
            // fn.Printf("%s_%d_%g_%04d", TheFileKey,bbtype, tau,dom.idx_out);
            fn.Printf("%s_%04d", TheFileKey, dom.idx_out);
            
            dom.WriteXDMF(fn.CStr());
            dom.idx_out++;
            // std::cout<<"--- Time = "<<dom.Time<<" ---"<<std::endl;
            // Report(dom,&my_dat); 
            tout += dtout;
        }
        // (dom.*dom.ptr2collide)();
        
        FreeSlip(dom,&my_dat);
        
                
        dom.CollideSRT();
        InOut(dom,&my_dat);
        dom.Stream();
        
        
        // dom.BounceBack(false);
        // dom.BounceBackQIBB(false);
        // (dom.*dom.ptr2bb)(true);
        dom.CalcProps();
        dom.Time += 1;
    }
    // dom.EndSolve();
    
    dom.EndTime = std::clock();
    double ttime = (double)(dom.EndTime - dom.StartTime)/CLOCKS_PER_SEC;
    printf("\033[01;34m---Elapsed Time = %f s---\033[0m\n",ttime);
    
    return 0;
}MECHSYS_CATCH