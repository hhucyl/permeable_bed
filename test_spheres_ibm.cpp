...
int main (int argc, char **argv) try
{
    ...
    char const * TheFileKey1 = "spheres_ibm";
    String fn2;
    fn2.Printf("%s_%d", TheFileKey1, h);
    fn2.append(".txt");
    std::fstream ifile1(fn2.CStr(),std::ios::in);
	int NN = 0;
    double Rc = 0;
    Vec3_t post(0.0,0.0,0.0);
    std::vector<Vec3_t> ppoints;
	if(!ifile1.fail())
	{
		ifile1>>Rc;
        ...
        ifile1>>NN;
        double ds = (M_PI*dx)/(3.0*NN)*(12.0*Rc*Rc + dx*dx);
    ...
    while(dom.Time<Tf)
    {
        ...
        {
            for(size_t in=0;in<dom.points.size(); in++)
            {
                dom.points[in] = ppoints[in] + center_pos[ip];
            }

            dom.ApplyIBM3D(center_pos[ip],R);
